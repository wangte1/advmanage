...
class Model_houses_points extends MY_Model {
    ...
    public function get_points_lists($where = array(), $order_by = array(), $pagesize = 0,$offset = 0,  $group_by = array()){
        ...
        $this->db->select('A.*, C.name as houses_area_name, A.addr, A.point_status, B.province, B.city, B.area,B.name AS houses_name, B.put_trade,B.grade,B.is_check_out, D.size');
