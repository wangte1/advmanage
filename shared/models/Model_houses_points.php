...
class Model_houses_points extends MY_Model {
    ...
    public function get_report_points($fields='*', $where = array(), $order_by = array(), $pagesize = 0,$offset = 0,  $group_by = array()){
        $this->db->select($fields);
        $this->db->from('t_houses_points A');
        $this->db->join('t_houses_points_report B','A.id = B.point_id','left');
        if(isset($where['like'])) {
            foreach($where['like'] as $k => $v) {
                $this->db->like($k, $v);
            }
