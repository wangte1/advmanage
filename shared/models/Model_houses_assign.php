...
class Model_houses_assign extends MY_Model {
    ...
    public function get_join_lists($where = array(), $order_by = array(), $pagesize = 0,$offset = 0,  $group_by = array()) {
        $this->db->select('A.*, B.province, B.city, B.area, B.name as houses_name,F.name as area_name,C.adv_img, C.order_code, C.order_type,C.release_start_time,C.release_end_time,C.customer_id, D.name as customer_name,E.fullname as charge_name');
