...
class Housesorders extends MY_Controller{
    ...
    public function get_points() {
        ...
        if(!empty($this->input->post('houses_type'))) $where['A.houses_type'] = $this->input->post('houses_type');
        ...
        $fields = 'A.id,A.code,A.houses_id,A.area_id,A.ban,A.unit,A.floor,A.addr,A.type_id,A.ad_num, A.ad_use_num, A.lock_num,A.point_status,A.houses_type';
        ...
        foreach ($points_lists as $k => $v){
            $points_lists[$k]['houses_type_name'] = '';
            foreach (C('zhiye') as $k1 => $v1){
                if($v['houses_type'] == $k1){
                    $points_lists[$k]['houses_type_name'] = $v1;
                }
            }
        }
        //获取去重的组团区域
    ...
    public function edit_customer_order($id=0){
        ...
        if(IS_POST){
            ...
            $id = $post_data['id'];
            unset($post_data['id'], $post_data['ban'], $post_data['unit'],$post_data['area_id'],$post_data['floor'], $post_data['addr'],$post_data['houses_type']);
            ...
            foreach ($data['selected_points'] as $k => $v){
                $data['selected_points'][$k]['houses_type_name'] = '';
                foreach (C('zhiye') as $k1 => $v1){
                    if($v['houses_type'] == $k1){
                        $data['selected_points'][$k]['houses_type_name'] = $v1;
                    }
                }
            }
    ...
    public function edit($id) {
        ...
        if (IS_POST) {
            ...
            //已选择点位列表
            $where['in']['A.id'] = explode(',', $data['info']['point_ids']);
            $houses_type = $this->input->get('houses_type');
            if($houses_type){
                $where['A.houses_type'] = $this->input->get('houses_type');
                $data['houses_type'];
            }
            $data['selected_points'] = $this->Mhouses_points->get_points_lists($where);
    ...
    public function detail($id){
        ...
        //换画记录
        $data['info']['change_pic_record'] = $this->Mhouses_changepicorders->get_order_lists(array('A.order_code' => $data['info']['order_code'], 'A.pid' => 0));
