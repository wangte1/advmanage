...
class Orders extends MY_Controller{
    ...
    public function index() {
        ...
        $data['list'] = $this->Morders->get_order_lists($where, ($page-1)*$pageconfig['per_page'], $pageconfig['per_page']);
        $data_count = $this->Morders->get_order_count($where);
    ...
    public function order_type() {
        $data = $this->data;
        $this->load->view('orders/order_type', $data);
    }
    ...
    public function add($order_type) {
        ...
        if (IS_POST) {
            ...
            unset($post_data['media_id'], $post_data['make_num'], $post_data['hour'], $post_data['minute'], $post_data['second']);
            $id = $this->Morders->create($post_data);
            if ($id) {
                ...
                $this->write_log($data['userInfo']['id'], 1, "新增".$data['order_type_text'][$post_data['order_type']]."订单,订单id【".$id."】");
                $this->success("添加成功！","/orders");
            } else {
                $this->success("添加失败！","/orders");
            }
        } else {
            $data['order_type'] = $order_type;

            //媒体列表
            $data['media_list'] = $this->Mmedias->get_lists("id, code, name", array('type' => $order_type, 'is_del' => 0), array('sort' => 'asc'));
    ...
    public  function ajax_update_status(){
        ...
        if($this->input->is_ajax_request()){
            ...
            //同时更新对应的订单
            $result = $this->Morders->update_info(array("order_status"=>$status),array("id"=>$id));
            if($status == 8){
                if($result){
                    //如果订单已经下画则释放所有点位
                    ...
                    //更新该订单下所有换画订单的状态为已下画
                    $order_code = $this->Morders->get_one('order_code', array('id' => $id))['order_code'];
                    $change_count = $this->Mchange_pic_orders->count(array('order_code' => $order_code));
                    if ($change_count) {
                        $this->Mchange_pic_orders->update_info(array('order_status' => 8), array('order_code' => $order_code));
                    }
                }
            }
    ...
    private function get_make_info($data) {
        //制作数量
        $make_num = $this->Mpoints->get_make_info(array('in' => array('A.id' => explode(',', $data['point_ids'])), 'C.order_id' => $data['id'], 'C.type' => 1));
    ...
    public function check_upload_img($order_id){
        $data = $this->data;
        $order = $this->Morders->get_one("*",array("id" => $order_id));
        if(IS_POST){
            $post_data = $this->input->post();
            ...
            $this->success("保存验收图片成功！","/orders/detail/".$order_id);
            exit;
        }

        //获取该订单下面的所有站台
        $points = $this->Mpoints->get_lists("media_id", array('in' => array("id" => explode(",",$order['point_ids']))));
