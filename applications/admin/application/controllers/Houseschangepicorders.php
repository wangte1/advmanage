...
//        $data['info']['make_company'] = $this->Mmake_company->get_one('company_name', array('id' => $data['info']['make_company_id']))['company_name'];
    ...
    public function insert_make_info($order_id) {
    	...
    	if(IS_POST) {
    		...
    		$res = $this->Mhouses_changepicorders->update_info($post_data, ['id' => $order_id]);
    		if($res) {
    			$this->success("录入制作信息成功！", "/houseschangepicorders/detail/".$order_id);
    		}else {
    			$this->error("操作失败！请重试！");
    		}
    	...
    	$data['info'] = $this->Mhouses_changepicorders->get_one('is_sample', ['id' => $order_id]);
