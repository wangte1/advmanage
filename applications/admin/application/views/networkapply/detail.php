    ...
    <div class="main-container-inner">
     	<div class="page-content">
					        ...
					        <tr>
					      	...
					      	<?php foreach($val1['networkinfo'] as $key2 => $val2) {?>
						        <tr>
						          ...
						          <?php for($i=0; $i<$val1['days']; $i++) {?>
						          		<?php $mark = false;?>
						          		<?php foreach($val1['apply_content'][$key2] as $key3 => $value3) {?>
						          	   		<?php if($value3['dateid'] == $i+1) {?>
						          	   			<?php $mark = true;?>
						          	   		<?php }?>
						          	   	<?php }?>
			                    	  	<?php if($mark == true) {?>
			                    	  		<?php if($list1[0]['status'] == 1) {?>
			                    	  			<td class="date-td date-order" style="background-color:#FF9900;">
			                    	  		<?php }else {?>
			                    	  			<td class="date-td date-order" style="background-color:#336699;">
							          	   	<?php }?>
							          	   	</td>
			                    	  	<?php }else{?>
					...
					<div style="float:left;">
						<span style="float:left;display:block;width:30px;height:20px;line-height:30px;color:#fff;text-align:center;background-color:#336699"></span><span style="float:left;">&nbsp;申请预定&nbsp;&nbsp;&nbsp;&nbsp;</span>
					</div>
					...
					<?php if($list1[0]['status'] == 0 && $role_auth == 1) {?>
					<center style="margin-top: 80px;">
						<button onclick="pass(<?php echo $list1[0]['id'];?>);" class="btn btn-sm btn-info">通过</button>
						<button onclick="noPass(<?php echo $list1[0]['id'];?>);" class="btn btn-sm btn-danger">不通过</button>
					</center>
					<?php }?>
		</div>
