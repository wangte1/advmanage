$(function(){
    ...
    function searchPoints() {
        var params = { media_type: $('input[name="order_type"]').val(), media_id:  $("#media_id").val()};
        $.post('/scheduledorders/get_points', params, function(data){
            $("#points_lists").empty();
            if (data.flag && data.points_lists.length > 0) {
                ...
                for (var i = 0; i < lists.length; i++) {
                    ...
                    html += '<td class="col-sm-2">' + lists[i]['point_status'] + '</td>';
